...
void ThermalPrinterComponent::print_bitmap(int w, int h, const uint8_t *bitmap, bool fromProgMem) {
  int rowBytes, rowBytesClipped, rowStart, chunkHeight, chunkHeightLimit, x, y, i;

  rowBytes = (w + 7) / 8; // Round up to next byte boundary
  rowBytesClipped = (rowBytes >= 48) ? 48 : rowBytes; // 384 pixel max width
  ...
  for (i = rowStart = 0; rowStart < h; rowStart += chunkHeightLimit) {
    ...
    this->write_bytes(ASCII_DC2, '*', chunkHeight, rowBytesClipped);

    for (y = 0; y < chunkHeight; y++) {
      for (x = 0; x < rowBytesClipped; x++, i++) {
        this->write_bytes(fromProgMem ? pgm_read_byte(bitmap + i) : *(bitmap + i));
      }
...
void ThermalPrinterComponent::print_bitmap(int w, int h, Stream *stream) {
  int rowBytes, rowBytesClipped, rowStart, chunkHeight, chunkHeightLimit, x, y, i, c;
...
void ThermalPrinterComponent::set_times(unsigned long x, unsigned long y) {
  // Not implemented for this printer
  ESP_LOGW(TAG, "set_times() not implemented for this printer model");
}

void ThermalPrinterComponent::tab_stops(const uint8_t *stops) {
  this->write_bytes(ASCII_ESC, 'D'); // Set tab stops
  for (int i = 0; i < 32; i++) {
    if (stops[i] == 0) break;
    this->write_bytes(stops[i]);
  }
  this->write_bytes(0);
}
