...
class JuraCoffeeComponent : public PollingComponent, public uart::UARTDevice {
 public:
  ...
  void update() override {
    ...
    if (result.length() < 39) { // 35 characters for cleanings + 4 for the value
        ...
        // Parse the data. The length check above prevents out-of-bounds errors.
 ...
 protected:
  // Jura communication function - closer to original with configurable timeout
  std::string cmd2jura(std::string outbytes) {
    ...
    while (!endsWith(inbytes, "\r\n")) {
      ...
      if (w++ > timeout_loops) { // Use configurable timeout
        return "";
      }
    }
